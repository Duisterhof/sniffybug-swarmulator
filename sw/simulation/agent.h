...
public:
  ...
  float laser_collision_threshold = 0.5; //if one of the laser rangers detects < 0.1 [m], we count it as a collision
  float swarm_collision_threshold = 0.5; // if two agents get within x[m], it's a crash for both agents
